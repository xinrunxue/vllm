...
elseif (ASIMD_FOUND)
    ...
    check_cxx_source_compiles("#include <arm_sve.h>\nint main() { svbool_t b = svptrue_b32(); return 0; }" HAS_SVE)
    
    # Check for SME support
    check_cxx_source_compiles("#include <arm_sme.h>\nint main() { asm volatile (\"smstart za\"); asm volatile (\"smstop za\"); return 0; }" HAS_SME)
    
    if(ARM_BF16_FOUND)
        message(STATUS "BF16 extension detected")
        if(HAS_SME)
            message(STATUS "SME extension detected")
            set(MARCH_FLAGS "-march=armv9-a+sve2+sme+bf16+dotprod+fp16")
            add_compile_definitions(ARM_BF16_SUPPORT ARM_SVE_SUPPORT ARM_SME_SUPPORT)
        elseif(HAS_SVE)
            message(STATUS "SVE extension detected")
            set(MARCH_FLAGS "-march=armv8.6-a+sve+bf16+dotprod+fp16")
            add_compile_definitions(ARM_BF16_SUPPORT ARM_SVE_SUPPORT)
        else()
        ...
        if(HAS_SME)
            message(STATUS "SME extension detected")
            set(MARCH_FLAGS "-march=armv9-a+sve2+sme+dotprod+fp16")
            add_compile_definitions(ARM_SVE_SUPPORT ARM_SME_SUPPORT)
        elseif(HAS_SVE)
            message(STATUS "SVE extension detected")
            set(MARCH_FLAGS "-march=armv8.6-a+sve+dotprod+fp16")
            add_compile_definitions(ARM_SVE_SUPPORT)
        else()
